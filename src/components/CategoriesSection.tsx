...
const CategoriesSection = () => {
  ...
  return (
    <div className="flex lg:h-screen py-20 gap-10 max-lg:justify-center">
      ...
      <div className="lg:w-1/3 flex flex-col justify-between py-10 text-text lg:mr-52 max-lg:space-y-10 max-lg:text-center ">
        <div className="max-lg:mx-[10%]">
          <h1 className="text-2xl">Categories</h1>
          <p className="font-bold text-5xl max-md:text-3xl">Furniture Sets Recommendations</p>
          <Accordion />
        </div>
        <div className="grid grid-cols-3 gap-5 lg:hidden max-lg:mx-[5%]">
          {images.map((image) => (
            <img src={image.url} alt={image.name} className="h-40 w-full object-cover rounded-lg" />
          ))}
        </div>
      </div>
    </div>
